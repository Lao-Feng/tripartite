(function ($) {
    ...
    $.learunindex = {
        ...
        loadMenu: function () {
            $(".main-sidebar").show();
            $("#myFrame").attr("src","/page/default.shtm");
            $("#user-info").hide();
            $(".content-tabs").css("left",0);
            $(".content-wrapper .content-tabs .roll-right").css("right",0);
            $(".content-wrapper .content-tabs .roll-right.tabRight").css("right",120);
            $(".content-wrapper .content-tabs .roll-right.btn-group").css("right",40);
            $("#firstView").text("欢迎首页");
            $(".content-wrapper").css("marginLeft",230)

            $.ajax({
                url: '/sys/user/getModules.shtm',
                type: 'get',
                dataType: 'json',
                success: function (list) {
                    ...
                    $.each(list, function (i) {
                        ...
                        if (row.parentId == 0) { //父级菜单
                            ...
                            _html += '<a href="#">';
                            _html += '<i class="fa fa-navicon"></i><span>' + row.moduleName + '</span><i class="fa fa-angle-left pull-right"></i>'
                            _html += '</a>';
                            var childNodes = $.learunindex.jsonWhere(list, function (v) { return v.parentId == row.moduleId });
                            if (childNodes.length > 0) {
                                _html += '<ul class="treeview-menu">';
                                $.each(childNodes, function (i) {
                                    var subrow = childNodes[i];
                                    var subchildNodes = $.learunindex.jsonWhere(list, function (v) { return v.parentId == subrow.moduleId });
                                    _html += '<li>';
                                    if (subchildNodes.length > 0) {
                                        _html += '<a href="#"><i class="fa fa-desktop"></i>' + subrow.moduleName + '';
                                        _html += '<i class="fa fa-angle-left pull-right"></i></a>';
                                        _html += '<ul class="treeview-menu">';
                                        $.each(subchildNodes, function (i) {
                                            var subchildNodesrow = subchildNodes[i];
                                            _html += '<li><a class="menuItem" data-id="' + subrow.moduleId + '" href="' + subrow.url + '"><i class="fa fa-desktop"></i>' + subchildNodesrow.moduleName + '</a></li>';
                                        });
                                        _html += '</ul>';

                                    } else {
                                        _html += '<a class="menuItem" data-id="' + subrow.moduleId + '" href="' + subrow.url + '"><i class="fa fa-desktop"></i>' + subrow.moduleName + '</a>';
                                    }
                                    _html += '</li>';
                                });
                                _html += '</ul>';
